...
| **DistilBERT (fine-tuned)** | *pending* | *pending* | 3-epoch CPU fine-tune on `evaluation/data/{train,val}.json`. Artifact `evaluation/artifacts/distilbert/` is **gitignored**. Run `evaluation/train_transformer()` then re-run `evaluation/evaluate.py` or check `evaluation/analyze_errors.py` for updated confusion. |
